...
sum(especiesA==0)/(sum(especiesA==0)+sum(especiesA!=0))
...
a1 <- rowSums(t(especiesA[especies$habitat == "cultivo",]))
...
totalE <- decostand(especiesA, "total", MARGIN=1)
totalE[1:5,2:4]
...
# sqrt(sum((ob1[1,]-ob1[2,])^2))
# sqrt(sum((ob1[1,]-ob1[3,])^2))
# sqrt(sum((ob1[2,]-ob1[3,])^2))
# sqrt(sum((ob1[1,]-ob1[4,])^2))
# sqrt(sum((ob1[2,]-ob1[4,])^2))
# sqrt(sum((ob1[3,]-ob1[4,])^2))
...
# Disimilitud de Gower con vegdist()
esG <- vegdist(especiesB, "gower")
head(esG)
head(sqrt(esG))
...
dev.new(title="Gower", width=10, height=5)
coldiss(esG, byrank=FALSE, diag=TRUE)

### Modo R --------------------------------------------
### Correlación en matrices
names(indices)

# Selección de variables explicativas numéricas
...
# Redondear correlaciones
round(cor(indicesA), 2)


par(mfrow = c(1,1))
par(pty = "s")
pairs(indicesA, lower.panel=panel.smooth, 
      upper.panel=panel.cor,
      diag.panel=panel.hist,
      main="Matriz de correlación de Pearson")

### Correlación de rho de Spearman
cor(indicesA, method = "spearman")

# Redondear correlaciones
round(cor(indicesA, method = "spearman"), 2)
...
cor(indicesA, method = "kendall")

round(cor(indicesA, method = "kendall"),2)

par(mfrow = c(1,1))
par(pty = "s")
pairs(indicesA, lower.panel=panel.smoothb, 
